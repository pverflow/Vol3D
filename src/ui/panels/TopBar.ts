...
export class TopBar {
  ...
  private readonly exportModal: ExportModal
  private readonly presetsMenu: PresetsMenu

  constructor(state: StateManager, presets: PresetManager) {
    this.helpModal = new HelpModal()
    this.exportModal = new ExportModal(state)
    this.presetsMenu = new PresetsMenu(state, presets)
    this.el = this.build(state)
    ...
    window.addEventListener('vol3d-show-export', () => this.exportModal.open())
  }

  private build(state: StateManager): HTMLElement {
    ...
    helpBtn.addEventListener('click', () => this.helpModal.open())
    ...
    presetsBtn.addEventListener('click', () => this.presetsMenu.open(presetsBtn))
    ...
    exportBtn.addEventListener('click', () => this.exportModal.open())
